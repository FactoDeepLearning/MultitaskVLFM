...
class SwinTransformer(nn.Module):
    ...
    def forward(self, seq, return_after=None):
        seq = self.patch_embed(seq)
        if self.ape:
            seq = seq + self.absolute_pos_embed
        ...
        layer = self.layers[-1]
        seq = layer.all_but_last_layer(seq)
        if return_after == "transformer-1":
            return seq, seq
        seq = layer.last_layer(seq)

        seq = self.norm(seq)  # B L C

        if return_after == "transformer":
            return seq, seq

        x = self.avgpool(seq.transpose(1, 2))  # B C 1
        x = torch.flatten(x, 1)
        x = self.head(x)
        return x, seq

    def proj(self, x, seq, features_from=None):
        last_tr = False
        ...
        if not (features_from == "transformer" and x is seq):
            x = self.avgpool(seq.transpose(1, 2))  # B C 1
            x = torch.flatten(x, 1)
        x = self.head(x)
        return x, seq
